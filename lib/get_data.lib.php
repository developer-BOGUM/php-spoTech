...
function get_content_by_field($write_table, $type='bbs', $where_field='', $where_value='', $is_cache=false)
{
    ...
    if( $type === 'content' ){
        $check_array = array('co_id', 'co_html', 'co_subject', 'co_sec_subject', 'co_content', 'co_seo_title', 'co_mobile_content', 'co_skin', 'co_mobile_skin', 'co_tag_filter_use', 'co_hit', 'co_include_head', 'co_include_tail');
    } else {
...
function get_thumbnail_find_cache($bo_table, $wr_id, $wr_key){
    ...
    return get_board_file_db($bo_table, $wr_id, 'bf_file, bf_content, bf_type', "and bf_type in (0, 1, 2, 3, 18) ", true);
}

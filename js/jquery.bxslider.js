...
;(function($){
	...
	var defaults = {
		...
		onSliderLoad: function($slideElement, oldIndex, newIndex){
			$('.list_'+ $slideElement + ' .bg img').addClass('bkg');
			playBar($slideElement);
		},
		onSlideBefore: function($slideElement, oldIndex, newIndex){
			playBar(newIndex);
		},
		onSlideAfter: function($slideElement, oldIndex, newIndex){
			$('.list_'+ oldIndex + ' .bg img').removeClass('bkg');
			$('.list_'+ newIndex + ' .bg img').addClass('bkg');
	...
	$.fn.bxSlider = function(options){
		...
		var appendControlsAuto = function(){
			...
			// add the controls to the DOM
			slider.controls.autoEl = $('<div class="bx-controls-auto" style="display: none;"/>');
			// bind click actions to the controls
...
function playBar(index) {
	clearInterval(id);
	$('.bar').css('width', '0');
	var ele= document.getElementById('swiper_' + index + '_bar');
	id = setInterval(frame, 65);
	$('.swiper-li').removeClass('active');
	$('#swiper_' + index).addClass('active');
	$('.btn-pause').data('val', index);
	var width = 0;
...
$(function(){
	$(".btn-pause").click(function(){
		if($(this).hasClass('play')) {
			...
			$('.list_'+ $(this).data('val') + ' .bg img').removeClass('back');
			playBar($(this).data('val'));
		}
		else {
			$("video").get(0).pause();
			$('.bx-stop').click();
			$(this).addClass('play');
			$('.bar').css('width', '0');
			$('.list_'+ $(this).data('val') + ' .bg img').addClass('back');
			clearInterval(id);
		}
	})
})
